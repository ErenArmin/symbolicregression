function gp=myconfig_BAIT2_fe3sol(gp)
...
gp.fitness.terminate_value = 0.0125;      %terminate run early if this fitness value or better is achieved
...
%% load data
load ("Gptips_CFe_BAIT2_openocean_upper_Southern.mat");
% Split data = 60:20:20
[r,c] = size(BAIT2) ; % r - rows, c - columns
...
BAIT2_train = BAIT2(idx(1:m),:) ; 
BAIT2_val = BAIT2(idx(m+1:n),:) ; 
BAIT2_test = BAIT2(idx(n+1:end),:) ;

% Normalize using TRAINING stats only
% Compute mean and sigma from training data
...
save("/Users/chengwangwang/Documents/MATLAB/CFemodel/Gptips_prerun_BAIT2_fe3sol_openocean_upper_Southern_normalized.mat",'BAIT2_train',...
...
%name for dataset
gp.userdata.name = 'PISCES-fe3sol-upper-Southern-normalized'; 
